	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < embedbits.length; i++) {
			 ...
			 pm.transToGray(pix2D, grayarray);// 轉灰階
			 pm.preprocess(grayarray, mark);// 溢位前處理
			 embedpointer = pm.embedImage(grayarray, embedbits, num,stegoArray);// 藏入
			 pm.saveImage(stegoArray, stegofilename);// 輸出偽裝影像
			 extractpointer = pm.extractImage(stegoArray, extract, num,recoverArray);// 擷取及還原*/
...
//				}
				...
				int sortedindex[] = new int[4];// 紀錄排序前位置
